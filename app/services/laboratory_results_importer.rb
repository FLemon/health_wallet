...
class LaboratoryResultsImporter
  ...
  def parse_header(values, line_number)
    ...
    { name: name, dob: Date.iso8601(dob), sex_at_birth: sex_at_birth, reference: reference, observations: [] }
  rescue Date::Error
  ...
  def parse_observation(values, line_number)
    ...
    { code: code, value: Float(value), units: units, name: OBSERVATION_NAMES.fetch(code) }
